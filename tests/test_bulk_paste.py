...
function fetchStub(url,opts){
  ...
  if(url==='/prefill'){
    ...
    return Promise.resolve({ok:true,status:200,json:()=>Promise.resolve({ok:true,values:{job_title:'T'}})});
  }
...
(async()=>{
  ...
  console.log(JSON.stringify({s:s,rows:rows,txt:B.summaryText(s,'')}));
})();
""")
    assert out["s"] == {"saved": 1, "failed": 2, "stopped": 0, "reviewIds": ["j-1"]}
    assert out["rows"]["0"] == {"status": "failed", "error": "<script>alert(1)</script> boom"}
    assert out["rows"]["1"] == {"status": "saved", "jobId": "j-1", "decision": "review", "score": "72"}
    assert out["rows"]["2"] == {"status": "failed", "error": "not a URL"}
    assert out["txt"] == "1 saved, 2 failed"
            ...
            def do_POST(self):
                ...
                if self.path == "/prefill":
                    if "bad-item" in form.get("job_url", ""):
                        ...
                        self._send(200, "application/json", json.dumps({"ok": True, "values": {"job_title": title}}).encode())
                elif self.path == "/job-submit":
                    self._send(303, "text/plain", b"", {"Location": f"/job/{form['job_title']}-1"})
                else:
    ...
    try:
        with sync_playwright() as pw:
            ...
            try:
                ...
                page.wait_for_function("document.getElementById('bulk-summary').textContent.includes('saved')")
                ...
                assert page.inner_text("#bulk-summary").startswith("2 saved, 1 failed")
                ...
                assert rows.nth(0).locator("a").get_attribute("href") == "/job/alpha-1"
                assert rows.nth(2).locator("a").get_attribute("href") == "/job/gamma-1"
                ...
                assert link.is_visible() and link.get_attribute("href") == "/review-queue?ids=alpha-1"
                assert errors == []
                ...
                assert page2.inner_text("#bulk-summary").startswith("1 saved, 0 failed, 2 not run (stopped)")
            finally:
                browser.close()
    finally:
        app.close()
