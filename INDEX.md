...
| `docs/tasks/source-quality-gating-design.md` | Source quality gating: <40 = skip blocker, ≥40 and <70 = force Review; thresholds in config. |
...
| `docs/tasks/career-ops-absorption-design.md` | Career-ops absorption (2026-07-08/09): qualitative assessment layer (LLM-judged dimensions, legitimacy tier, idempotent CAS on-demand route), A-F grade (decision-aware + culture-capped), persisted `eval_queue` batch assessment. Implemented slices 1-3; comp dimension and bulk paste input excluded/deferred. |
...
| `src/job_hunt_scoring.py` | `score_job(profile, job, policy)` → `ScoringResult`. 7-component weighted scoring on the match_score scale 0–100 (weights sum to 100): required skills (35pt), preferred skills (5pt), experience (20pt), location (10pt), salary (10pt), domain (10pt), work mode (10pt). Confidence (high/medium/low) derived independently from data completeness — NOT from score. Unknown fields get neutral credit but reduce confidence. Risk flags (`missing-required-skills`, `missing-preferred-skills`, `salary-below-floor`) built here. |
| `src/job_hunt_scoring_presets.py` | **NEW (Slice C, 2026-07-21 search/score/filter plan, 2026-07-22).** 3 named `ScoringWeights` presets — `PRESET_BALANCED` (today's defaults, unchanged), `PRESET_SALARY_FOCUSED`, `PRESET_SKILLS_FOCUSED` — each validated to sum to 100 via `ScoringWeights.__post_init__`. `load_preset_name`/`save_preset_name` (atomic temp-file + `fsync` + `os.replace` write; corrupt/missing file falls back to Balanced) and `get_active_scoring_policy(state_root=)` used by `ui_handlers`, `job_hunt_orchestrator.py:125`, and `job_hunt_scheduler.py:168,358`. |
| `src/job_hunt_decision.py` | `decide_application(score, blockers, risk_flags, policy)` → `DecisionResult`. Priority: blockers → skip; score ≥80 + no critical risks → apply; score ≥65 and <80 → review; critical risks present → review (even with high score); else → skip. |
...
| `src/job_hunt_scheduler.py` | **(Daily Digest D3/D5/D6 + OQ-2 + career-ops slice 3).** D3: `run_digest_pipeline -> DigestRunResult` (LLM-free pipeline; `_PIPELINE_LOCK` serialises with manual Run-now). D5: `DigestScheduler` daemon (poll loop, once-per-day at `digest_run_time`, exception-isolated, lock-guarded `status()`). D6: `drain_llm_batch` (worker-lock paced Gemini, 429→backoff+requeue, RPD cap, source-aware detail), `LLMQueueWorker` daemon, `save_analysis_llm_fields`, `rpd_date_key` (Pacific), `llm_queue_stats`. **OQ-2:** `reevaluate_digest_jobs -> ReevalResult` (under `_PIPELINE_LOCK`; re-score every digest row, crossed-up→resurface+requeue, dropped-below→dequeue), `row_status` helper. **Career-ops slice 3 (2026-07-09):** `process_eval_queue_once -> EvalQueuePollResult` folded into `LLMQueueWorker`'s loop as a single shared dispatcher (digest first, then ≤1 `eval_queue` row per cycle). First committed to version control this session (existed on disk earlier but untracked). |
| `src/job_hunt_qualitative.py` | **NEW (career-ops absorption slice 1, 2026-07-08; grade added slice 2, 2026-07-09).** `build_qualitative_prompt(jd_text, profile)`, `parse_and_validate(...)` (strict-JSON schema, fail-closed), `run_qualitative_assessment_pipeline(...)`. Grade: `derive_base_grade(match_score)`, `derive_grade(...)` (decision-aware + culture cap), `apply_grade_to_assessment(...)`. `QualitativeRunResult`/`QualitativeValidationFailure`/`QualitativeValidationError`. |
| `src/text_grounding.py` | **NEW (career-ops absorption slice 1).** `normalize_grounding_text(text)`, `quote_in_text(quote, source_text)` — shared evidence-quote validation (whitespace/casing/Unicode-punctuation folding); reused by CV tailoring's bullet validator rather than duplicated. |
...
| `src/ui_routes.py` | **NEW (LT-1); hardened 2026-07-27.** HTTP server + dispatch. `UIRequest` (parsed method/path/query/form/json_body/raw_body/headers), `UIResponder` (`send_html`/`send_json`/`redirect`), `_parse_request`, `_build_handler` (slim `do_GET`/`do_POST` → standalone handlers), `main`, `build_parser`. Loopback-only bind validation, 1 MiB POST cap before body reads, and same loopback host/port browser POST Origin/Referer checks; errors do not expose internals. Imports each `job_sources/*_source` for registration side effect. |
| `src/ui_handlers.py` | **NEW (LT-1).** All request handlers as standalone `handle_*(req, config, responder)` / `render_*` functions (testable without a live server). View-model builders `_build_job_page_vm` / `_build_profile_page_vm`, `_render_search_jobs_tab` (source orchestration), `_allowed_profile_dir`, `load_recent_job_history`, `raw_input_payload_from_form`, `parse_multipart_form`, `_index_db_path`, `handle_sources`. **Career-ops slices 1-3 (2026-07-08/09):** `_ensure_qualitative_grade`, `handle_qualitative_assess` (`POST /job/{id}/qualitative-assess`); `_batch_assess_selected_ids`, `handle_batch_assess` (`POST /jobs/batch-assess`), `handle_get_batch` (`GET /batch/{batch_id}`), `handle_cancel_batch` (`POST /batch/{batch_id}/cancel`). Only layer that imports domain modules. |
| `src/ui_render.py` | **NEW (LT-1); updated 2026-07-27.** All HTML rendering — **pure: data in, string out, no domain imports**. View-models `JobPageViewModel`, `ProfilePageViewModel`, `ReviewQueueViewModel`; `render_board_page()` renders escaped stage columns, counts, cards, and empty states. `render_page` (reads `config.model_label`), `render_home_page`, `render_job_page`, `render_profile_page`, `render_review_queue_page`, `render_history_table`, `render_input_form`, `_render_sidebar`, `_render_add_job_*`, `_normalize_home_tab`; responsive rules keep the tabbed shell, forms, grids, and board usable at narrow widths. **Career-ops slices 1-3:** nested `_render_qualitative_panel` (evidence quotes, legitimacy tier, Gemini privacy disclosure), `_grade_badge`, `_grade_warning_banner` inside `render_job_page`; review-queue checkboxes for batch-assess selection. |
...
| `src/ui_chip_field.py` | **NEW (Slice D, 2026-07-21 search/score/filter plan, 2026-07-22).** Vanilla-JS chip/tag input, progressive enhancement only: `render_chip_field(name, label, value, ...)` renders a real name-carrying `<input>` (no-JS submission unchanged, parsed server-side by `ui_handlers._parse_keyword_terms`/`_parse_exclude_terms`); `CHIP_FIELD_JS` (one shared `<script>` block, like `_multiselect.py`'s pattern) hides the input and renders removable pills, re-writing the comma-joined value back into the same hidden field on every add/remove. Used by `_render_shared_search_form` for keywords/exclude/location. |
...
| `src/job_sources/relevance.py` | **NEW (Slice A, 2026-07-21 search/score/filter plan, 2026-07-22).** Local lexical relevance filter. `bucket_jobs_by_relevance(jobs, query) -> (matches, other)` — never hard-drops; role-family rule: `_tokenize`/`_stem` (coarse suffix-stem so analyst/analysts/analysis/analyses collapse), `_derive_role_terms` (stemmed query words + an acronym from the query's own initials, e.g. "Business Analysis" → "ba"). `is_relevant_title` (primary signal) / `is_relevant_description` (weak, unstemmed secondary signal). Wired into `ui_handlers.handle_source_search`. |
| `src/job_sources/search_state.py` | **NEW (Slice B, 2026-07-21 search/score/filter plan, 2026-07-22).** In-memory per-search seen-key state for "Show more" pagination, safe under `ThreadingHTTPServer` concurrency: `start_search(seen_jobs) -> search_id` (random `uuid4().hex`, resets on each new search), `filter_new_page(search_id, page_jobs)` (drops already-shown jobs; fails open — never over-drops — if the search-id is unknown/expired), `known_search`. `threading.Lock`-guarded; TTL expiry (`SEARCH_STATE_TTL_SECONDS`=3600) + hard cap (`MAX_TRACKED_SEARCHES`=500) via `_prune_locked`. Reuses `job_sources.dedup.is_duplicate_job` for the identity check. |
...
| `src/job_hunt_salary_benchmark.py` | **NEW (Feature B, 2026-09-22).** Pure advisory helpers: `clean_job_title(title)` (strips seniority/contract/location/company noise), `summarise_histogram(buckets, job_salary, floor)` (median bucket, total, percentile rank of job salary and salary floor). Never feeds scoring/decision/grade. |
| `src/job_hunt_export.py` | **NEW (Feature C, 2026-09-22).** Application package zip (stdlib only, in memory). `validate_job_id` (allow-list `^[A-Za-z0-9._-]+$`, max 128, no `.`/`..`), `package_filename`, `render_analysis_md`, `build_package(job_id, profile, *, state_root, policy, cover_letter_base, now)` -> `(zip_bytes, manifest)`. Fixed entries `README.txt`, `cv.md`, `cover_letter.txt`, `analysis.md`, `job.json`; missing parts are listed in the README/manifest, only a missing job raises. Never includes `.env`, `raw_inputs`, profile personal fields. |
| `src/job_sources/adzuna_client.py` | **CHANGED (Feature B, 2026-09-22).** Added `fetch_adzuna_salary_histogram(what, *, location, cache_dir, now)` -> `SalaryHistogramResult` (status ok/no_data/error/unavailable). 24h file cache under `data/state/cache/adzuna_salary/` (gitignored), persisted daily call counter (stops at 200/day), 10s timeout, fixed-text errors (never the URL or app key), no raw-response write. |
...
| `src/job_sources/linkedin_source.py` | **NEW (P5-2); hardened 2026-07-11; MEDIUM fixes 2026-07-17.** LinkedIn public-search scraper (no API key). `LinkedInBlockedError`; `is_available` (always True); SQLite cache (5-min TTL, SHA-256 key, `set_cache_db_path`; `_cache_get` deletes the stale row on TTL miss; `search_handler` always caches — empty results included); `normalize_search_params` (`None`-safe keywords/location); `_is_blocked` (HTTP 999/429/403/401, `login`+`authwall` redirect URLs, page < 5000 chars); `_parse_search_html` (BeautifulSoup `.base-card`, job-id dedup, `description_preview`); `_fetch_search` (lazy `requests`); `search_handler` (cache-first); `_fetch_description` (lazy full desc); `select_handler` (form validation, URL origin check — rejects non-LinkedIn URLs, skill extraction, returns `default_form_values`); `_linkedin_job_id`; `render_search_form`; `_render_select_form`, `_render_cards` (scheme-validates href — blocks `javascript:` XSS), `render_results` (mirrors Adzuna `.jst-rc` pattern); `_register()`. Salary always empty and `salary_min_gbp`/`salary_max_gbp` always `None` (not available on public search). |
...
| `tests/test_adzuna_salary.py` | Feature B salary benchmark (2026-09-22): client (ok/no_data/429/timeout/malformed/missing creds, no key in errors or logs), cache hit + TTL, daily budget 200, `summarise_histogram`, `clean_job_title`, route handler (400/404/200/unavailable/502), rendered panel. No real network. |
| `tests/test_bulk_paste.py` | Bulk paste on Add & Evaluate (2026-09-22): rendered controls, node-run split/classify + loop logic, Playwright stub-app and real-endpoint runs (Playwright cases skip without it). |
| `tests/test_export_bundle.py` | Feature C export package (2026-09-22): full / CV-only / nothing-generated zips (`testzip()` clean, manifest of missing parts), analysis.md text vs fixture, profile-mismatch CV missing, secrets/raw_inputs/profile PII absent, traversal ids rejected with no file access, cover-letter save/load round trip from a tmp cwd, `UIResponder.send_bytes` + header-injection rejection, route 400/404/200 for Apply/Review/Skip, job-page button. |
| `tests/test_ui.py` | UI request handling plus live-browser smoke coverage: Add Job activation and mobile 390×844 layout across Find Jobs, Add Job, Evaluate, Digest, Profile, and Board View. |
...
| `tests/test_ui_handlers_uncovered.py` | **NEW (2026-07-08; expanded 2026-07-27).** Handler regressions, including Board View delegation to `render_board_page`; LLM/parse boundaries mocked. |
...
| Board aggregate (GAP-H) | `gap-h-board-aggregate-design.md` | `job_hunt_index.py`: `load_board`; `ui_handlers`: `handle_get_jobs`, `handle_get_board_view`, `handle_jobs_save`; `ui_render.render_board_page` | `GET /jobs`, `GET /board`, `GET /board/view`, `POST /jobs/save` | Tracker tab |
| CV tailoring (GAP-F) | `cv-tailoring-brief.md` | `tailor_cv`, `TailoredCVResult`, `validate_tailored_cv` (fail-closed normalised achievement grounding), `save_tailored_cv` | `POST /tailor` | Tailor CV workspace |
...
| LinkedIn source (P5-2) ✅ | — | `linkedin_source.py`: `is_available`, `normalize_search_params`, `search_handler`, `select_handler` (URL origin validation), `_fetch_search`, `_fetch_description`, `_is_blocked` (login+authwall), `_parse_search_html`, `_linkedin_job_id`, `render_search_form`, `_render_cards` (scheme-safe href), `render_results`, SQLite cache helpers; `LinkedInBlockedError` | `GET /search/linkedin`, `POST /select/linkedin` | Find Jobs tab |
...
| Profile load/save/CV parse | — | `load_candidate_profile`, `save_candidate_profile`, `parse_cv_file`; `render_profile_page` sectioned preferences/evidence/CV settings | `GET /profile`, `POST /profile/save`, `POST /profile/parse-cv` | My Profile tab (no local CV paths rendered) |
| Job detail page | — | `render_job` (`ui_handlers`), `render_job_page` (`ui_render`) | `GET /job/{id}` | Job detail page |
| Salary benchmark (F5, advisory) | `salary-benchmark-2026-09-22.md` | `fetch_adzuna_salary_histogram` (`adzuna_client`); `clean_job_title`, `summarise_histogram` (`job_hunt_salary_benchmark`); `handle_job_salary_benchmark` (`ui_handlers`); `render_salary_benchmark_panel` (`ui_render`) | `GET /job/{id}/salary-benchmark` | Job page -> Salary benchmark |
| Bulk URL/JD paste (CAREER-F1) | `bulk-paste-2026-09-22.md` | `_BULK_PASTE_JS` (`classify`, `processItem`, `runAll`) in `src/ui_render.py`; reuses `handle_prefill`, `handle_job_submit` | `POST /prefill`, `POST /job-submit` (no new route) | Add & Evaluate → Bulk |
| Application package export (F4) | `export-bundle-2026-09-22.md` | `build_package`, `validate_job_id`, `package_filename`, `render_analysis_md` (`job_hunt_export`); `load_cover_letter`, `cover_letter_dir` (`job_hunt_cover_letter`); `handle_job_export` (`ui_handlers`); `UIResponder.send_bytes` (`ui_routes`) | `GET /job/{id}/export.zip` | Job page -> Download package |
| Guided manual intake + advanced review | — | `_intake_form_with_job_id`, `_run_intake_evaluation`, `handle_job_submit`, `handle_evaluate` | `POST /prefill`, `POST /job-submit`, `POST /evaluate` | Add & Evaluate → Advanced Review |
...
| Qualitative assessment (career-ops slice 1) ✅ | `career-ops-absorption-design.md` | `job_hunt_qualitative.py`: `build_qualitative_prompt`, `parse_and_validate`, `run_qualitative_assessment_pipeline`; `text_grounding.py`: `normalize_grounding_text`, `quote_in_text`; `job_hunt_index.py`: `claim_qualitative_assessment`, `finish_qualitative_assessment`, `reserve_llm_rpd_attempt`; `ui_handlers.handle_qualitative_assess`; `ui_render._render_qualitative_panel` | `POST /job/{id}/qualitative-assess` | Job detail page |
| A-F grade badge (career-ops slice 2) ✅ | `career-ops-absorption-design.md` | `job_hunt_qualitative.py`: `derive_base_grade`, `derive_grade`, `apply_grade_to_assessment`; `job_hunt_index.update_qualitative_grade`; `ui_render._grade_badge`, `_grade_warning_banner` | — | Job detail page |
| Batch qualitative assessment queue (career-ops slice 3) ✅ | `career-ops-absorption-design.md` | `job_hunt_index.py`: `enqueue_eval_batch`, `claim_eval_queue_row`, `finish_eval_queue_row`, `requeue_eval_queue_row`, `return_eval_queue_row_to_pending`, `cancel_eval_batch`, `reset_stale_eval_queue_running`, `pause_eval_batch_for_quota`, `get_eval_batch`, `get_eval_queue_stats`; `job_hunt_scheduler.process_eval_queue_once` (shared dispatcher inside `LLMQueueWorker`); `ui_handlers`: `handle_batch_assess`, `handle_get_batch`, `handle_cancel_batch` | `POST /jobs/batch-assess`, `GET /batch/{batch_id}`, `POST /batch/{batch_id}/cancel` | Review Queue (selection) → Batch progress page |
| Relevance filter — "Other results" bucket (Slice A) ✅ | `2026-07-21-search-score-filter-plan.md` | `job_sources/relevance.py`: `bucket_jobs_by_relevance`, `is_relevant_title`, `is_relevant_description`, `_derive_role_terms`, `_stem`; wired into `ui_handlers.handle_source_search` before dedup | `GET /search/{source}` (bucketing only, no new route) | Find Jobs tab (collapsed "Other results" `<details>` section) |
| Live-search dedup + cross-page seen-state (Slice B) ✅ | `2026-07-21-search-score-filter-plan.md` | `job_sources/dedup.py`: generalized `_identity_fields`/`is_duplicate_job`/`deduplicate_jobs` (supports both `NormalizedJob` and the UI search-result dict); `job_sources/search_state.py`: `start_search`, `filter_new_page`, `known_search` (lock-guarded, TTL + cap); wired into `ui_handlers.handle_source_search` and `handle_source_search_more` | `GET /search/{source}`, `GET /search/{source}/more` | Find Jobs tab |
| Scoring-weight presets (Slice C) ✅ | `2026-07-21-search-score-filter-plan.md` | `job_hunt_scoring_presets.py`: `PRESET_WEIGHTS` (Balanced/Salary-focused/Skills-focused), `load_preset_name`, `save_preset_name` (atomic JSON write), `get_active_scoring_policy`; `ui_handlers.handle_set_scoring_preset`; `ui_render.render_profile_page` ("Scoring weights" section); called from `job_hunt_orchestrator.py:125`, `job_hunt_scheduler.py:168,358` | `POST /scoring-preset` | My Profile tab |
| Multi-keyword search + chip entry (Slice D) ✅ | `2026-07-21-search-score-filter-plan.md` | `ui_handlers._parse_keyword_terms`, `_run_multi_keyword_search` (one search per keyword term, same location/radius, capped at 6 sub-searches); `ui_chip_field.py`: `render_chip_field`, `CHIP_FIELD_JS` (progressive enhancement, no-JS fallback); `_render_shared_search_form` (keywords/exclude/location chip fields) | `GET /search/{source}` (multi-keyword parsing, no new route) | Find Jobs tab |
