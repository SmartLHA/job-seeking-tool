...
### ⬜ SSF-F1 · "Show more" doesn't carry the Other-results bucket forward
**Status:** ⬜ Pending
`handle_source_search` buckets the first page into `matches`/`other` (relevance filter, Slice A) and renders "Other results" as a collapsed section, but `handle_source_search_more` only re-runs the search and dedups — it does not re-apply `bucket_jobs_by_relevance` or extend the "Other results" section with later pages. Later pages' non-matching jobs are currently just shown in the main list.

### ⬜ SSF-F2 · Multi-keyword pagination shares one cursor across keywords
**Status:** ⬜ Pending
`_run_multi_keyword_search` (Slice D) runs one sub-search per keyword term and merges results, but `handle_source_search_more`'s "Show more" uses a single shared skip/take cursor across all keyword terms rather than one cursor per term — a "Show more" click after a multi-keyword search does not evenly page through every term's remaining results.

### ⬜ SSF-F3 · Browser click-test of chip add/remove not done
**Status:** ⬜ Pending
`src/ui_chip_field.py`'s vanilla-JS chip widget (add on comma/Enter, remove via the × button, Backspace to pop the last chip) is covered by the no-JS hidden-field parsing tests (`tests/test_multi_keyword_search.py`) but the JS interaction itself has not been exercised in a real browser (e.g. via the Chrome MCP or computer-use). Low risk (progressive enhancement, no-JS path already tested) but flagged as unverified UI behaviour.
