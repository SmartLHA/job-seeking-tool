...
**Status:** ✅ Done 2026-09-20 — spec `docs/tasks/audit-fixes-2026-09-20.md`; source audit `docs/audits/ui-functions-audit-2026-09-19.md`.
...
**Status:** ✅ Done 2026-06-27 — updated 2026-07-11 (security hardening) — updated 2026-07-17 (4 MEDIUM code-review fixes) — 44 tests green (`tests/test_linkedin_source.py`)
**2026-07-17 MEDIUM fixes:** `salary_min_gbp`/`salary_max_gbp` unified to `None` (was a `None`/`""` mix); empty search results now always cached (`search_handler`); stale cache rows deleted on TTL miss (`_cache_get`); `normalize_search_params({"keywords": None})` no longer crashes. +4 tests (TTL expiry, error-HTML escaping, `None` keyword/location). No scraping/scoring changes.
**What was built:**
1. `src/job_sources/linkedin_source.py` — full `JobSource` adapter (public scraper, no API key): `LinkedInBlockedError`, SQLite cache (5-min TTL, SHA-256 key), `_is_blocked` (HTTP 999/429/403/401 + login-redirect URL + `authwall` redirect + short-page heuristic), `_parse_search_html` (BeautifulSoup `.base-card`, job-id dedup), `search_handler` (cache-first), lazy `_fetch_description` + `select_handler` (URL origin validation — rejects non-LinkedIn URLs), XSS-safe card rendering (`_render_cards` — scheme check blocks `javascript:` hrefs), `_register()` import-time side effect.
2. `tests/test_linkedin_source.py` — 44 tests covering: card parsing, blocked variants (login-wall/authwall/short-page/429/403/timeout), empty results, XSS escaping (title + javascript-URL href), dedup, param normalisation/clamping/`None` keyword+location, cache-hit skips HTTP, cache TTL expiry (stale row deleted → re-fetch), render edge cases + error-HTML escaping, `select_handler` (18 tests: happy path, wrong source, field-length limits, work_mode/employment_type normalisation, lazy fetch triggered/skipped/failed, config handling, source_ref/job_id logic), URL origin check (4 tests).
...
## 🆕 Career-ops Absorption — Qualitative Assessment + A-F Grade + Batch
**Design doc:** `docs/tasks/career-ops-absorption-design.md` (decisions confirmed by Mike 2026-07-08; Codex-reviewed).

Ports the useful parts of the career-ops (santifer/career-ops, MIT) evaluation model — LLM-judged qualitative dimensions, a letter-grade presentation, and a resumable batch queue — onto the existing deterministic `score_job()`/`decide_application()` pipeline. The qualitative layer is advisory only: it never changes `match_score` or the Apply/Review/Skip decision.

### ✅ Slice 1 · Qualitative assessment layer
**Status:** ✅ Done 2026-07-08 — `tests/test_qualitative.py` (18) + 10 qualitative route tests in `tests/test_ui.py`.
**What:** `src/job_hunt_qualitative.py` (prompt build, strict-JSON schema validation, fail-closed on malformed output) + `src/text_grounding.py` (shared evidence-quote normalisation, reused by CV tailoring); `qualitative_index` SQLite table with an atomic CAS claim so concurrent requests can't double-claim; `POST /job/{id}/qualitative-assess` (idempotent, `force` semantics); assessment panel with a Gemini privacy disclosure; quota now reserved per Gemini attempt (incl. failures/429s), not only on success.
**Effort:** M

### ✅ Slice 2 · A-F grade badge
**Status:** ✅ Done 2026-07-09.
**What:** `derive_base_grade`/`derive_grade`/`apply_grade_to_assessment` in `job_hunt_qualitative.py` — deterministic band from `match_score` (A≥80/B 72-79/C 65-71/D 50-64/F<50), capped to the effective-decision band (Apply→A, Review→B, Skip→D, blocker-Skip→F) AND to the qualitative culture/red-flags cap; always displayed base→capped+reason; warning banner for base A/B with culture ≤2. AMENDED mid-slice (Codex HIGH): decision cap added after finding a score-only grade could show "A" beside a Skip.
**Effort:** S

### ✅ Slice 3 · Persisted batch assessment queue + progress UI
**Status:** ✅ Done 2026-07-09 — `tests/test_eval_queue.py` (10).
**What:** `eval_queue`/`eval_batch` SQLite tables; `process_eval_queue_once` folded into the existing `LLMQueueWorker` as a single shared dispatcher (digest first, then ≤1 eval row per cycle — avoids two independent Gemini workers racing quota); `POST /jobs/batch-assess` (review-queue selection only), `GET /batch/{batch_id}` progress page, `POST /batch/{batch_id}/cancel` (pending rows only; a running job finishes its in-flight call with a pre-persist cancel check); stale-claim reset every poll, not just at worker startup.
**Effort:** M
...
| Career-ops slice 1 | Qualitative assessment layer — `job_hunt_qualitative.py`, `text_grounding.py`, `qualitative_index` CAS claim, `POST /job/{id}/qualitative-assess`, 28 new tests (18 in test_qualitative.py + 10 route tests in test_ui.py) | ✅ 2026-07-08 |
| Career-ops slice 2 | A-F grade badge — decision-aware + culture-capped derivation, base→capped+reason display, warning banner | ✅ 2026-07-09 |
| Career-ops slice 3 | Persisted batch assessment queue — `eval_queue`/`eval_batch`, shared `LLMQueueWorker` dispatcher, `POST /jobs/batch-assess`, `GET /batch/{batch_id}`, `POST /batch/{batch_id}/cancel`, 10 new tests | ✅ 2026-07-09 |

---

## Follow-ups — Search triage (2026-07-02)

### ✅ TRIAGE-F1 · Tailor Apply regression
**Status:** ✅ Done 2026-07-27 — Apply decisions now return a tailored CV after fail-closed achievement grounding; route regression coverage is green.
The former 422 was resolved during the Critical remediation. The unrelated working-tree note is retained as historical context only.

### ⬜ TRIAGE-F2 · Bounded look-ahead refill for fully-hidden pages
**Status:** ✅ Done 2026-08-10 — bounded server-side look-ahead checks up to three additional pages only when the requested page is fully hidden or excluded; it preserves Other results and per-keyword cursors.
A requested page whose jobs are all hidden or excluded now checks at most three further source pages. It stops as soon as it finds visible main or Other-results cards, reaches source exhaustion, or reaches the fixed cap; it never performs an unbounded refill.

### ⬜ TRIAGE-F3 · Search/filter inside the Hidden jobs overlay
**Status:** ✅ Done 2026-08-10 — client-side title/company filter, live count and empty state added without mutating the hidden-jobs store; browser smoke coverage passed.
...
**Status:** ✅ Done 2026-08-10 — confirmed reset is available only for mistakenly saved `rejected`/`withdrawn` outcomes. It returns the job to `not_applied` while preserving the original terminal event and reset reason in history.
...
**Status:** ✅ Done 2026-09-22 — spec `docs/tasks/bulk-paste-2026-09-22.md`. "Bulk" mode on Add & Evaluate: client-side sequential loop over existing `/prefill` + `/job-submit`, max 15 items, no LLM assessment queued (assessment stays the explicit Review-queue step). Tests `tests/test_bulk_paste.py`.
...
**Status:** ✅ Done 2026-09-22 — spec `docs/tasks/export-bundle-2026-09-22.md`. Job page "Download package" -> `GET /job/{id}/export.zip`: stdlib in-memory zip with `README.txt` (manifest incl. missing parts), `cv.md`, `cover_letter.txt`, `analysis.md`, `job.json`. New `src/job_hunt_export.py`, `load_cover_letter`/`cover_letter_dir` shared with save, `UIResponder.send_bytes`. Tests `tests/test_export_bundle.py`. No PDF/DOCX (browser "Save as PDF").

### ✅ F5 · Adzuna salary benchmark panel (advisory)
**Status:** ✅ Done 2026-09-22 — spec `docs/tasks/salary-benchmark-2026-09-22.md`. Job page "Salary benchmark" panel, lazy button "Check market salary" -> `GET /job/{id}/salary-benchmark` (Adzuna UK histogram, nationwide, title-based). Shows sample size, median range, percentile of the job's midpoint salary and of the salary floor, "as of" date, Source: Adzuna. 24h file cache + 200/day call budget. Tests `tests/test_adzuna_salary.py`.
**Terms note:** Adzuna terms allow personal research use; Adzuna is acknowledged as the source in the panel; data is not for redistribution or publication (publishing aggregates needs Adzuna's written consent). Advisory only: no effect on score, decision or grade. Caveat: histogram may include estimated salaries (UNVERIFIED).

### ⬜ CAREER-F2 · Comp-vs-market dimension
**Status:** ⬜ Pending
Excluded from the port (`docs/tasks/career-ops-absorption-design.md` §1) because it needs live market-salary data this tool doesn't fetch (no web scraping/research). The existing deterministic salary-vs-floor check remains the only comp signal. Revisit only if a salary-data source is approved.
...
**Status:** ✅ Done 2026-08-10 — raw-response, worker, lock and parser checks now assert observable behaviour.
...
**Status:** ✅ Done 2026-08-10 — the test mocks `dotenv.load_dotenv`, so it reaches the missing-file fallback deterministically without consulting a real `.env`.
...
**Status:** ✅ Re-baselined 2026-08-10 — the historical 14-function snapshot is superseded. Full-suite coverage is 81% across the whole checkout, with the low aggregate dominated by the separate `viewer/` application; current Job Seeking Tool source modules are broadly covered. Added a default-reset-reason regression while rechecking the new outcome path. Future coverage work should target a named module/behaviour, not revive the stale percentage list.
...
## Follow-ups — Search/Score/Filter improvements (2026-07-22)

### ✅ SSF-F1 · "Show more" preserves the Other-results bucket
**Status:** ✅ Done 2026-07-28 — each subsequent page repeats relevance bucketing, deduplication, filters, and cross-page seen-state handling for both the main and Other-results buckets.

### ✅ SSF-F2 · Multi-keyword pagination uses independent cursors
**Status:** ✅ Done 2026-07-28 — continuation URLs carry a bounded per-keyword cursor vector. Exhausted terms are not queried again; a failed term retains its cursor for retry.

### ✅ SSF-F3 · Real-browser chip add/remove/Backspace coverage
**Status:** ✅ Done 2026-07-29 — `tests/test_ui.py` exercises Enter/comma add, button remove, and Backspace removal, verifying the hidden submitted input after each action. The smoke test passed in the system browser runtime with no page errors.
