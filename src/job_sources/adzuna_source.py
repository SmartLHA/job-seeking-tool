...
All Adzuna-specific rendering, normalisation and registration logic, mirroring
:mod:`src.job_sources.reed_source` (the reference implementation). Self-contained:
the UI layer discovers it only through
:mod:`src.job_sources.source_registry`.

Two deliberate differences from Reed:

* **No source snapshot.** The reviewed-input pipeline only requires/validates a
  ``source_snapshot`` for ``source_type == "reed"`` (see
  ``raw_input_payload_from_form`` in ``ui_handlers``), so Adzuna does not build
  or carry one.
* **No detail fetch on select.** The Adzuna client exposes only a search call
  (no per-job detail endpoint), so the description captured from the search
  result is what is used for evaluation. Adzuna search descriptions are already
  largely complete; they are capped at the shared 500-char select-form limit.
"""
...
from src.job_sources.normalize import normalize_adzuna
...
def _ensure_adzuna_env_loaded() -> None:
    """Load .env so ADZUNA_APP_ID/ADZUNA_APP_KEY are visible (mirrors Reed)."""
    import os
...
def default_adzuna_search_values() -> dict[str, str]:
    return {
        "keywords": "",
        "locationName": "",
        "minimumSalary": "",
...
def normalize_adzuna_search_params(params: dict[str, str]) -> dict[str, str]:
    values = default_adzuna_search_values()
...
def search_adzuna_jobs_for_ui(search_values: dict[str, str]) -> list[dict[str, Any]]:
    ...
    return [adzuna_job_to_ui_result(raw, search_values) for raw in raw_jobs[:take]]


def adzuna_job_to_ui_result(raw_job: dict[str, Any], search_values: dict[str, str]) -> dict[str, Any]:
    normalized = normalize_adzuna(raw_job)
    salary_display = _format_adzuna_salary_display(
        normalized.get("salary_min"), normalized.get("salary_max"), normalized.get("salary_text")
    )
    description = squash_whitespace(normalized.get("description") or "")
    return {
        "source": "adzuna",
        "source_job_id": _normalize_adzuna_source_job_id(normalized.get("external_id")),
        ...
        "salary_min_gbp": _normalize_adzuna_salary_value(normalized.get("salary_min")),
        "salary_max_gbp": _normalize_adzuna_salary_value(normalized.get("salary_max")),
        "employment_type": normalized.get("contract_type") or "Unknown",
        "work_mode": normalized.get("remote_type") or "Unknown",
        "url": normalized.get("apply_url") or normalized.get("original_url") or None,
        "description_preview": description[:280],
        "description_raw": _truncate_adzuna_description(description),
        "filter_notes": _adzuna_filter_notes(normalized, search_values),
    }


def _normalize_adzuna_source_job_id(value: Any) -> str | None:
    text = str(value or "").strip()
    return text if text and text.lower() not in {"none", "null", "unknown"} else None


def _normalize_adzuna_salary_value(value: Any) -> str:
...
def _truncate_adzuna_description(description: str) -> str:
    text = description or ""
    if len(text) <= 500:
        return text
...
def _format_adzuna_salary_display(salary_min: Any, salary_max: Any, salary_text: Any = None) -> str:
    if salary_text:
        return str(salary_text)
    min_value = _parse_adzuna_salary_number(salary_min)
    max_value = _parse_adzuna_salary_number(salary_max)
...
def _parse_adzuna_salary_number(value: Any) -> int | None:
    if value is None or value == "":
        return None
    try:
...
def _adzuna_filter_notes(normalized: dict[str, Any], search_values: dict[str, str]) -> list[str]:
    notes: list[str] = []
    if search_values.get("minimumSalary"):
        notes.append("Minimum salary is shown as a best-effort display filter; it is not sent to Adzuna in this PL.")
    ...
    if requested_work_mode != "any" and actual_work_mode != requested_work_mode:
        notes.append(f"Requested work mode '{requested_work_mode}' was not confirmed by Adzuna fields; shown as '{actual_work_mode}'.")
    ...
    if requested_employment != "any" and actual_employment != requested_employment:
        notes.append(f"Requested employment type '{requested_employment}' was not confirmed by Adzuna fields; shown as '{actual_employment}'.")
    if not notes:
        notes.append("Adzuna result normalized for display; filters are best-effort.")
    return notes


# ── search form ───────────────────────────────────────────────────────────────

def _render_adzuna_search_form(values: dict[str, str], enabled: bool) -> str:
    disable_attr = "" if enabled else ' style="opacity:0.5;pointer-events:none;"'
    return f"""
    <form method="get" action="/search/adzuna" id="adzuna-search-form" class="panel subtle"{disable_attr}>
      <h3>Search Adzuna</h3>
      ...
      <p class="prefill-status">Remote/hybrid, employment type and minimum salary are applied best-effort; unsupported filters are called out in result notes.</p>
      <div class="actions">
        <button type="submit">Search Adzuna</button>
...
# ── select form (hidden per-card POST form) ───────────────────────────────────

def render_adzuna_select_form(result: dict[str, Any], nonce: str | None, form_id: str | None = None) -> str:
    # Adzuna needs a source_job_id or url plus a description to be evaluable.
    if not ((result.get("source_job_id") or result.get("url")) and result.get("description_raw")):
    ...
    # nonce kept as a placeholder hidden field for parity with Reed (not validated).
    ...
    return f'<form method="post" action="/select/adzuna" class="actions adzuna-select-form"{id_attr} style="display:none;">{"".join(hidden)}</form>'


def adzuna_selected_job_id(cleaned: dict[str, str]) -> str:
    base = cleaned.get("source_job_id") or f"{cleaned.get('title', '')}-{cleaned.get('company', '')}"
    slug = re.sub(r"[^a-z0-9]+", "-", base.lower()).strip("-")[:80]
    return f"adzuna-{slug or uuid.uuid4().hex[:12]}"


def _validate_adzuna_salary_text(value: str, field_name: str) -> str:
    if not value:
        return ""
...
def adzuna_select_form_to_evaluate_values(form: dict[str, str], config: "object | None" = None) -> dict[str, str]:
    ...
    for key, limit in _SELECT_FORM_FIELD_LIMITS.items():
        ...
        cleaned[key] = value
    if cleaned["source"] != "adzuna":
        raise ValueError("This handler only processes Adzuna search results")
    ...
    salary_min = _validate_adzuna_salary_text(cleaned["salary_min_gbp"], "salary_min_gbp")
    salary_max = _validate_adzuna_salary_text(cleaned["salary_max_gbp"], "salary_max_gbp")

    # Adzuna has no per-job detail endpoint; the search description is final.
    full_description = cleaned["description_raw"]
    advert_url = cleaned["url"] or ""
    source_job_id = cleaned["source_job_id"]
    # source_ref keeps the advert URL when known, else the bare Adzuna id.
    source_ref = advert_url or source_job_id

    _req_skills, _pref_skills, _skill_warn = extract_skills_from_text(full_description)
    if _skill_warn:
        logger.warning("Adzuna select skill extraction: %s", _skill_warn)

    values = default_form_values()
    values.update(
        {
            "job_id": adzuna_selected_job_id(cleaned),
            "input_method": "adzuna_search",
            "job_url": advert_url,
            "source_type": "adzuna",
            ...
            "notes": "Prefilled from Adzuna search result. Review all fields before evaluation.",
        }
    )
    return values


# ── results rendering (multi-select grid; mirrors Reed) ───────────────────────

def _render_adzuna_cards_fragment(
    ...
    *,
    ...
    for i, result in enumerate(results):
        ...
        form_html = render_adzuna_select_form(result, nonce, form_id=form_id)

        # Selectable only when the job can actually be evaluated (identifier +
        # description). Otherwise render disabled so it can't be shortlisted.
        selectable = bool(
            (result.get("source_job_id") or result.get("url")) and result.get("description_raw")
        )
...
def render_adzuna_search_results(
    results: list[dict[str, Any]] | None,
    *,
    adzuna_error: str | None = None,
    adzuna_select_nonce: str | None = None,
    ...
    if adzuna_error:
        return (
            ...
            '<div style="font-size:14px;font-weight:700;color:var(--skip);margin-bottom:6px);">Adzuna search unavailable</div>'
            f'<div style="font-size:13px;color:var(--ink-soft);">{escape(adzuna_error)}</div>'
    ...
    if not results:
        return (
            ...
            '<div style="font-size:15px;font-weight:600;color:var(--ink-soft);">No Adzuna results found</div>'
    ...
    cards_html = _render_adzuna_cards_fragment(results, skip=0, nonce=adzuna_select_nonce)
    ...
    return (
        ...
        f'text-transform:uppercase;color:var(--ink-faint);">Adzuna results ({total_str})</span>'
...
def _register() -> None:
    register(JobSource(
        source_id="adzuna",
        display_name="Adzuna",
        is_available=_is_adzuna_available,
        normalize_search_params=normalize_adzuna_search_params,
        search_handler=search_adzuna_jobs_for_ui,
        select_handler=adzuna_select_form_to_evaluate_values,
        render_search_form=_render_adzuna_search_form,
        render_results=lambda results, error, nonce, more_url=None: render_adzuna_search_results(
            results, adzuna_error=error, adzuna_select_nonce=nonce, more_url=more_url
        ),
        render_cards_fragment=lambda results, *, skip=0, nonce=None: _render_adzuna_cards_fragment(
